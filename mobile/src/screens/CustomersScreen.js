  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <FlatList data={customers} renderItem={renderItem} keyExtractor={i => i.id.toString()}
        ...
        ListEmptyComponent={<View style={{ alignItems: 'center' }}><Text style={{ fontSize: 60 }}>👥</Text><Text style={{ color: colors.textLight, marginTop: 12 }}>No customers yet</Text></View>}
      />
    </SafeAreaView>
  );
}
...
const getStyles = (colors) => StyleSheet.create({
  ...
  search: { flex: 1, backgroundColor: colors.card, borderRadius: 10, paddingHorizontal: 16, paddingVertical: 12, fontSize: 14, elevation: 2 },
  addBtn: { marginLeft: 10, width: 48, height: 48, borderRadius: 24, backgroundColor: colors.primary, justifyContent: 'center', alignItems: 'center', elevation: 3 },
  addBtnText: { color: colors.card, fontSize: 28, fontWeight: 'bold', marginTop: -2 },
  card: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.card, marginHorizontal: 16, marginBottom: 8, borderRadius: 12, padding: 14, elevation: 1 },
  ...
  avatarText: { color: colors.card, fontSize: 18, fontWeight: 'bold' },
